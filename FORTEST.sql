...
SELECT SUBSTR(ADDRESS,1,INSTR(ADDRESS,'광역시',1,1)-1)||'시'
FROM TEST
WHERE ADDRESS LIKE '%광역시';
...
UPDATE TEST
SET ADDRESS = (SELECT SUBSTR(ADDRESS, 1, 2) || '시' FROM TEST)
WHERE ADDRESS LIKE '%광역시';

/*
 * CONCAT('문자열1', '문자열2') : '두 문자열'을 하나의 문자열로 연결
 */

UPDATE TEST
SET ADDRESS = CONCAT(SUBSTR(ADDRESS,1,2),'시')
WHERE ADDRESS LIKE '%광역시';
...
FROM EMPLOYEE
WHERE ROW_NUM <=3; --ORA-00904: "ROW_NUM": invalid identifier
/*
 * ORA-00904: "ROW_NUM": invalid identifier
 * ROW_NUM 별칭 사용불가
 * 별칭 사용 불가 이유? 오라클 실행순서 때문에
...
FROM EMPLOYEE
WHERE ROWNUM <=3;
--SELECT 절 이전에 WHERE 절에서 ROWNUM을 먼저 연산처리 한 이후 SELECT 절 안의 RANK절을 연산했기 때문에 원하는 결과가 안나옴
...
SELECT *
FROM(SELECT ROWNUM, ENAME, SALARY, RANK() OVER(ORDER BY SALARY DESC, ENAME ASC) AS "등수"
	 FROM EMPLOYEE) E
WHERE "등수" <= 3;
